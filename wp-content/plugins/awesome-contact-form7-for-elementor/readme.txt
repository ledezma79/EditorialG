=== Awesome Contact Form7 for Elementor ===
Contributors: rafiul17, tbyte
Donate link: https://www.paypal.com/donate?hosted_button_id=ZC2N2PY77T9HL
Tags: Elementor, addons, contact-form, contact, contact-form-7
Requires at least: 6.3
Tested up to: 6.6.2
Stable tag: 3.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
 
Add Awesome Contact Form7 for Elementor with easy way.  


== Description ==
Adds Awesome Contact Form7 for Elementor that are specifically designed to be used in conjunction with the Elementor Page Builder and Contact form7.

Video tutorial link:

[youtube https://youtu.be/47vsPWSUXPY]

Make a cool contact form in 5 minutes with  Awesome Contact Form7 for Elementor

[youtube https://youtu.be/s9YWe3j47n4]




== Installation ==


== Changelog ==

= 3.1 =
*Release Date - 10 October, 2024*

 - Resolved security issue
 - Tested on latest WordPress & Elementor

= 3.0 =
*Release Date - 23 May, 2024*

 - Resolved security issue
 - Tested on latest WordPress & Elementor

= 2.9 =
*Release Date - 06 Dec, 2023*

 - Added Select dropdown style
 - Tested on latest verion of WordPress and Elementor

= 2.8 =
*Release Date - 09 Oct, 2023*

 - Added Input type File Style options

= 2.6 =
*Release Date - 21 Sept, 2023*

 - Tested on latest Elementor 3.16.4 and WordPress 6.3.1
 - Typography added for button style
 - Fix some minor bug

= 2.5 =
*Release Date - 08 March, 2023*

 - Tested on latest Elementor 3.11.3 and WordPress 6.1.1
 - Padding option added from button
 - CSS fixed for input type tel
 - Fix some minor bug


= 2.3 =
*Release Date - 21 November, 2022*

 - Tested on latest Elementor 3.8.1 and WordPress 6.1.1
 - Fix some bug
 - Fix the Admin notification never show message.
 
= 2.2 =
*Release Date - 13 october, 2022*

 - Tested on latest Elementor 3.7.8 and WordPress 6.0.2
 - Fix some bug
 - Fix the Admin notification never show message.

= 1.9 =
*Release Date - 08 october, 2021*

- Fix some bug
- Fix the Admin notification never show message.
 
= 1.8 =
*Release Date - 19 September, 2021*

 - Fix the Admin notification never show message.
 
= 1.6 =
*Release Date - 8th September, 2021*

 - Fix front-end edit bug.
 - Fix Elementor deprecated functions warning.
 - Check on WordPress version 5.8
 - Check on latest Elementor version 3.4.3

= 1.5 =
*Release Date - 22 January, 2021*

- Fix date input style
- Add Title and  sub title in form container
- Add Title and  sub title style option

= 1.4 =
*Release Date - 16th December, 2020*

- Add Margin control in button 
- Add Input text color option 

= 1.3 =
*Release Date - 15th December, 2020*

- Fix a CSS issue

= 1.2 =
*Release Date - 29th August, 2020*

- Add button hover 

= 1.1=
*Release Date - 30th September, 2019*

- Add form style
- Add new CSS property
- Fix some bug

= 1.0 =
*Release Date - 20th June, 2019*

* Initial Release